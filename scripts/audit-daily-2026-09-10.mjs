...
const errors = [];
const expected = {
  "boat-trailer-transom-straps-guide.html": { affiliate: 3, hub: "gear.html", card: "assets/editorial/boat-transom-straps-photo-card.webp" },
  "otsego-lake-family-boating.html": { affiliate: 0, hub: "destinations.html", card: "assets/editorial/otsego-lake-family-planning-photo-card.webp" },
  "boat-engine-overheat-warning-response.html": { affiliate: 0, hub: "journal.html", card: "assets/editorial/boat-engine-overheat-response-photo-card.webp" },
};

if (publication20260910.length !== 3) errors.push(`expected exactly 3 source pages, found ${publication20260910.length}`);
...
for (const page of publication20260910) {
  ...
  if (page.slug === "boat-trailer-transom-straps-guide.html" && !/Compare short marine transom ratchet straps on Amazon/.test(html)) errors.push(`${page.slug}: category-specific affiliate labels are missing`);
...
const photoKeys = [
  "boat-transom-straps-photo-hero", "boat-transom-straps-photo-card",
  "otsego-lake-family-planning-photo-hero", "otsego-lake-family-planning-photo-card",
  "boat-engine-overheat-response-photo-hero", "boat-engine-overheat-response-photo-card",
];
...
if (errors.length) {
  console.error(`Daily 2026-09-10 audit failed with ${errors.length} problem(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}
console.log("Daily 2026-09-10 audit passed: 3 substantial pages, 6 photographic assets, 3 disclosed affiliate links and permanent feed/sitemap discovery.");
