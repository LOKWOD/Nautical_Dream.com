...
const errors = [];
const expected = {
  "boat-trailer-security-coupler-wheel-lock-guide.html": { affiliate: 3, hub: "gear.html", card: "assets/editorial/boat-trailer-security-layers-photo-card.webp" },
  "saratoga-lake-family-boating.html": { affiliate: 0, hub: "destinations.html", card: "assets/editorial/saratoga-lake-family-plan-photo-card.webp" },
  "used-boat-title-hin-paperwork-guide.html": { affiliate: 0, hub: "journal.html", card: "assets/editorial/used-boat-paperwork-photo-card.webp" },
};

if (publication20260916.length !== 3) errors.push(`expected exactly 3 source pages, found ${publication20260916.length}`);
...
for (const page of publication20260916) {
  ...
  if (page.slug === "boat-trailer-security-coupler-wheel-lock-guide.html") {
    for (const label of ["Compare boat-trailer coupler locks on Amazon", "Compare locking hitch-receiver pins on Amazon", "Compare boat-trailer wheel clamps on Amazon"]) if (!html.includes(label)) errors.push(`${page.slug}: missing category-specific affiliate label: ${label}`);
    if (!/delay and deterrence tools/i.test(html) || !/security never raises/i.test(html)) errors.push(`${page.slug}: deterrence or towing-rating boundary missing`);
  }
  if (page.slug === "saratoga-lake-family-boating.html" && (!/Route 9P/i.test(html) || !/40 cars and trailers/i.test(html) || !/illustrative, not documentary/i.test(html))) errors.push(`${page.slug}: launch, parking or illustrative-image boundary missing`);
  if (page.slug === "used-boat-title-hin-paperwork-guide.html" && (!/model year 1987 or newer/i.test(html) || !/at least 14 feet/i.test(html) || !/One wrong character changes the identity/i.test(html))) errors.push(`${page.slug}: New York title or HIN identity boundary missing`);
...
const photoKeys = [
  "boat-trailer-security-layers-photo-hero", "boat-trailer-security-layers-photo-card",
  "saratoga-lake-family-plan-photo-hero", "saratoga-lake-family-plan-photo-card",
  "used-boat-paperwork-photo-hero", "used-boat-paperwork-photo-card",
];
...
if (errors.length) {
  console.error(`Daily 2026-09-16 audit failed with ${errors.length} problem(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}
console.log("Daily 2026-09-16 audit passed: 3 substantial pages, 6 photographic assets, 3 disclosed affiliate links and complete discovery.");
