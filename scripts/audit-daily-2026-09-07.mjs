...
const errors = [];
const expected = {
  "boat-trailer-winch-strap-guide.html": { affiliate: 3, hub: "gear.html", card: "assets/editorial/boat-trailer-winch-photo-card.webp" },
  "sodus-bay-family-boating.html": { affiliate: 0, hub: "destinations.html", card: "assets/editorial/sodus-bay-planning-photo-card.webp" },
  "crossing-boat-wakes-family.html": { affiliate: 0, hub: "journal.html", card: "assets/editorial/family-wake-crossing-photo-card.webp" },
};

if (publication20260907.length !== 3) errors.push(`expected exactly 3 source pages, found ${publication20260907.length}`);
const newTitles = new Set();
const newSlugs = new Set();
const allHtml = readdirSync(root).filter((file) => file.endsWith(".html"));

for (const page of publication20260907) {
...
const photoKeys = [
  "boat-trailer-winch-photo-hero", "boat-trailer-winch-photo-card",
  "sodus-bay-planning-photo-hero", "sodus-bay-planning-photo-card",
  "family-wake-crossing-photo-hero", "family-wake-crossing-photo-card",
];
...
if (errors.length) {
  console.error(`Daily 2026-09-07 audit failed with ${errors.length} problem(s):`);
