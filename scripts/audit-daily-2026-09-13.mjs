...
const errors = [];
const expected = {
  "boat-battery-box-tray-hold-down-guide.html": { affiliate: 3, hub: "gear.html", card: "assets/editorial/boat-battery-securement-photo-card.webp" },
  "cranberry-lake-family-boating.html": { affiliate: 0, hub: "destinations.html", card: "assets/editorial/cranberry-lake-family-planning-photo-card.webp" },
  "gasoline-odor-boat-response.html": { affiliate: 0, hub: "journal.html", card: "assets/editorial/gasoline-odor-response-photo-card.webp" },
};

if (publication20260913.length !== 3) errors.push(`expected exactly 3 source pages, found ${publication20260913.length}`);
...
for (const page of publication20260913) {
  ...
  if (page.slug === "boat-battery-box-tray-hold-down-guide.html") {
    for (const label of ["Compare vented marine battery boxes with hold-down straps on Amazon", "Compare marine battery trays with crossbar hold-downs on Amazon", "Compare marine battery terminal protective boots on Amazon"]) if (!html.includes(label)) errors.push(`${page.slug}: missing category-specific affiliate label: ${label}`);
    if (!/less than one inch/i.test(html)) errors.push(`${page.slug}: federal movement boundary missing`);
  }
  if (page.slug === "cranberry-lake-family-boating.html" && (!html.includes("44.220621") || !html.includes("55 cars and trailers") || !html.includes("illustrative, not documentary"))) errors.push(`${page.slug}: launch facts or illustrative-image label missing`);
  if (page.slug === "gasoline-odor-boat-response.html" && (!/No spark/i.test(html) || !/Do not operate the bilge pump/i.test(html))) errors.push(`${page.slug}: no-spark or bilge-pump boundary missing`);
...
const photoKeys = [
  "boat-battery-securement-photo-hero", "boat-battery-securement-photo-card",
  "cranberry-lake-family-planning-photo-hero", "cranberry-lake-family-planning-photo-card",
  "gasoline-odor-response-photo-hero", "gasoline-odor-response-photo-card",
];
...
if (errors.length) {
  console.error(`Daily 2026-09-13 audit failed with ${errors.length} problem(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}
console.log("Daily 2026-09-13 audit passed: 3 substantial pages, 6 photographic assets, 3 disclosed affiliate links and complete discovery.");
